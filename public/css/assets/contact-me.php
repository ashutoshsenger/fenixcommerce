...
if($_POST) {
    ...
    $to_Email = 'akhilesh@fenixcommerce.com'; // Write your email here to receive the form submissions
    $subject = 'New message from FenixCommerce'; // Write the subject you'll see in your inbox

    $name = $_POST["userName"];
    $email = $_POST["userEmail"];
    $cname = $_POST["userCompany"];
    ...
    if(!isset($_POST["userChecking"]) || !isset($_POST["userName"]) || !isset($_POST["userEmail"]) || !isset($_POST["userCompany"]) ) {
    ...
    if(empty($_POST["userName"])) {
        $output = json_encode(array('type'=>'error', 'text' => '<span>Please Enter your Name!'));
        die($output);
    }
    
    if(!filter_var($_POST["userEmail"], FILTER_VALIDATE_EMAIL)) {
        $output = json_encode(array('type'=>'error', 'text' => '<span>Please Enter your email address.'));
        die($output);
    }

    if(empty($_POST["userCompany"])) {
        $output = json_encode(array('type'=>'error', 'text' => '<span>Error! Please enter the company name.'));
        die($output);
    }
    ...
    $headers .= 'From: Fenix Commerce <noreply@yourdomain.com>' . "\r\n";
                        ...
                        <tr>
                            <td>
                                    ...
                                    <table cellpadding='0' cellspacing='0' border='0' align='center'  style='width: 100%;max-width:600px;background:#FFFFFF;margin:0 auto;border-radius:5px;padding:50px 30px'>
                                        ...
                                        <tr>
                                            <td width='100%' align='left' style='padding-bottom:30px;'>
                                                <div>
                                                    <p>Hello, you've just received a request for demo!.</p>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                ...
                                <div style='margin-top:30px;text-align:center;color:#b3b3b3'>
                                    <p style='font-size:12px;'>2018 FenixCommerce®, All Rights Reserved.</p>
                                </div>
                            </td>
                        </tr>
                    </table>
    ...
    if(!(mail($to_Email, $subject, $emailcontent, $headers))) {
